...
//create some values to keep track of certain things (making an object)
const defaultValues = {
    gameStarted: false,
...
//generate and overwrite html into the DOM, which in turn generates
const generateGameBoard = () => {
    const dimensions = DOMElements.board.getAttribute("data-dimension");
    if (dimensions % 2 !== 0) {
        throw new Error(
            "The dimension of the baord MUST be an even whole number."
            );
    }
    const emojis = [
        "🍎",
...
const flipCard = (card) => {
    ...
    if (defaultValues.allCardsCurrentlyFlipped === 2) {
        ...
        if (
            allCardsCurrentlyFlipped[0].innerText === allCardsCurrentlyFlipped[1].innerText
        ) {
    ...
    if (!document.querySelectorAll(".card:not(.flipped").length) {
        setTimeout(() => {
            ...
            DOMElements.win.innerHTML = `
                <span class="win-text">
                    YOU'VE WON!<br /><br />
                    MOVES:<span class="highlight">
                    ${defaultValues.totalFlips}</span><br /><br />
                    TIME:<span class="highlight">
                    ${defaultValues.timeElapsed}s</span>
                </span>
            `;
...
const addEventListeners = () => {
    document.addEventListener("click", (e) => {
        ...
        if (
            target.className.includes("card") && !target.className.includes("flipped")
        ) {
            flipCard(parent);
        } else if (
            target.nodeName === "BUTTON" && !target.className.includes("disabled")
        ) {
            startGame();
